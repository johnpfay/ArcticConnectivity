...
; CD, 'C:\Workspace\Gits\ArcticConnectivity\Data\PolarScienceCenter\PIOMAS\IDLscripts'
pro hi, ps=ps
ncol = 1 & nrow=1
!p.multi = [0, ncol, nrow]
...
vlon=fltarr(nx,ny, /NOZERO) & vlat=fltarr(nx,ny, /NOZERO)
       ...
       close,9 & openr, 9, openfile00(0)
       readf, 9, alpha, format='(10f8.2)'
       close,9

for file_ct=0, data_source-1  do begin
       openr, file_ct+1, openfile(file_ct), /swap_if_big_endian
       openr, file_ct+10, openfile1(file_ct), /swap_if_big_endian

       for iii=0,n_month-1 do begin
           readu, file_ct+1, data3
           readu, file_ct+1, data4
...
; Define color table.
gun1=[  0, 60,  0,255,  0,  0,255,230,0]
gun2=[  0,180,  0,  0,255,  0,255,230,255]
gun3=[  0,240,  0,  0,  0,192,  0,230,255]
tvlct,gun3,gun2,gun1
white=8 & blue=1 & red=2 & red=3 & green=4 & darkblue=5 & yellow=6 & brown=7
...
;for j=0,ny-1,3 do begin
  ...
  oplot, [lon(i,j),xhead], [lat(i,j),yhead], max_value=99990.0, color=red
...
if iregion eq 1 then begin
  ...
  oplot, [lon3(0),xhead,lon3(1)], [lat3(0),yhead,lat3(1)], max_value=99990.0, color=red, thick=5.0
endif else if iregion eq 2 then begin
...
; Draw a vector scale
  ...
  oplot, [xtail,xhead], [ytail,yhead], $
         max_value=99990.0, color=red, thick=2

if iregion eq 1 then begin
  LLtoXY, [ytail,yhead], [xtail,xhead], xx1, yy1, 0.0
  heads, xx1(0), yy1(0), xx1(1), yy1(1), xx3, yy3, xx4, yy4, len=0.3, ang=25
  XYtoLL, [xx3,xx4], [yy3,yy4], lat3, lon3, 0.0
  oplot, [lon3(0),xhead,lon3(1)], [lat3(0),yhead,lat3(1)], max_value=99990.0, color=red, thick=2.0
endif else if iregion eq 2 then begin
  LLtoXY_s, [ytail,yhead], [xtail,xhead], xx1, yy1, 0.0
  heads, xx1(0), yy1(0), xx1(1), yy1(1), xx3, yy3, xx4, yy4, len=0.3, ang=25
  XYtoLL_s, [xx3,xx4], [yy3,yy4], lat3, lon3, 0.0
  oplot, [lon3(0),xhead,lon3(1)], [lat3(0),yhead,lat3(1)], max_value=99990.0, color=blue, thick=2.0
endif

  if iregion eq 1 then begin
     xyouts, xlon+4.0, xlat+2.2, '0.1 m/s', color=green, $
          charsize=1.1, charthick=3
  endif
  if iregion eq 2 then begin
     xyouts, xlon-5.0, xlat+3.0, '0.1 m/s', color=orange, $
          charsize=1.1, charthick=3
  endif
  if iregion eq 3 then begin
     xyouts, xlon-5.0, xlat-6.0, '0.1 m/s', color=red, $
          charsize=1.1, charthick=3
  endif
...
;xyouts, 110, 68, filestr, color=red, charsize=2.7, charthick=3, alignment=0.5
;xyouts, -50, 68, string(format='(f6.2)',vmean), color=red, charsize=1.0, charthick=2, alignment=0.5
