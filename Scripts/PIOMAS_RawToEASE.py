...
for year in range(2012,2013):
    ...
    for month in range(2,3):
        ...
        outFN = "XPts{}{}.shp".format(year,strMonth)
        ...
        #Add fields
        print "   ...Adding fields"
        arcpy.AddField_management(outFC,"X","FLOAT",10,8)
        arcpy.AddField_management(outFC,"Y","FLOAT",10,8)
        ...
        cursor = arcpy.da.InsertCursor(outFC,['SHAPE@XY','Angle','U','V','Lng','Lat','X','Y'])
        for x in range(yDim):
            for y in range(xDim):
                theLat = latArr[x,y]
                theLng = lngArr[x,y]
                ...
                theU = sliceU[x,y]
                theV = sliceV[x,y]
                #Convert from curvilinear to spherical
                theRadians = theAngle * math.pi / -180
                theX = (theU * math.cos(theRadians)) + (theV * math.sin(theRadians))
                theY = (theU * math.cos(theRadians)) - (theV * math.sin(theRadians)) 
                theRec = ((theLng,theLat),theAngle,theU,theV,theLng,theLat,theX,theY)
        ...
        if not(createASCII or writeRasters):
            continue
